...
def test_prepare_m3_tts_input_adds_boundary_hints_and_preserves_timing_hints(tmp_path: Path) -> None:
    ...
    second_segment = payload["segments"][1]

    assert first_segment["source_timing_hints"]["trailing_silence_seconds"] == 0.01
    assert second_segment["source_timing_hints"]["leading_silence_seconds"] == 0.01
    assert abs(first_segment["boundary_hints"]["gap_to_next_seconds"] - 0.05) < 1e-6
    assert abs(second_segment["boundary_hints"]["gap_from_prev_seconds"] - 0.05) < 1e-6
    assert second_segment["boundary_hints"]["continuation_risk_prev"] >= 0.5
    assert first_segment["boundary_hints"]["boundary_cut_risk_score"] > 0.0
